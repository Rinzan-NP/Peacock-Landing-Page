...
export default function About() {
    return (
        <div className="pt-32 pb-20 bg-white">
            {/* Hero — editorial split: large headline left, stats right */}
            <section className="bg-primary mx-4 md:mx-8 mb-28 relative overflow-hidden shadow-2xl rounded-2xl">
                <div className="absolute inset-0 bg-[url('/hero.png')] opacity-[0.08] mix-blend-overlay bg-cover bg-center" />
                <div className="absolute -right-24 -top-24 w-[500px] h-[500px] rounded-full bg-accent/20 blur-3xl pointer-events-none" />
                ...
                <div className="container relative z-10 mx-auto px-8 md:px-14 py-20 md:py-28 grid lg:grid-cols-12 gap-12 items-end">
                    ...
                    <FadeIn className="lg:col-span-7">
                        ...
                        <p className="font-body text-white/65 text-base leading-relaxed max-w-lg">
                            Peacock is a leading outsourcing services provider in the Middle East with a proven track record —
                            offering engineering, training, CAD outsourcing, equipment rentals, printing, project support, and HR recruitment.
                        </p>
                    </FadeIn>

                    {/* Right: compact credential list */}
                    <FadeIn delay={0.2} className="lg:col-span-5">
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <div
                                    key={i}
                                    className="bg-white/10 border border-white/15 rounded-xl p-5 flex flex-col text-center hover:bg-white/15 transition-colors"
                                >
            ...
            {/* Philosophy — left text, right values list, no box around both */}
                ...
                <div className="grid lg:grid-cols-2 gap-16 items-start">
                    <FadeIn>
                        ...
                        <p className="font-body text-slate-500 text-base leading-relaxed mb-5">
                            We are committed to provide quality services to our customers thereby helping them achieve their
                            business objectives in a faster, easier and cost-effective way.
                        </p>
                        <p className="font-body text-slate-500 text-base leading-relaxed">
                            Well trained, well experienced and well motivated, our human resource team will complete tasks on a
                            turnkey basis — no compromise on quality under any circumstances. Specialists in Oil & Gas,
                            Power Transmission & Distribution, and MEP engineering.
                        </p>
                    </FadeIn>
            ...
            {/* Legacy — stripped-back, typographic milestone list */}
            <section className="bg-slate-50 border-y border-slate-100 py-24">
                <div className="container mx-auto px-6 max-w-7xl">
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {[
                            { year: "2005", title: "Founded", desc: "Started as a dedicated MEP and Power Transmission specialist serving the Middle East." },
                            { year: "→", title: "Expanding Horizons", desc: "Established Peacock Resources LLP to bring global expertise in Digital Construction to India." },
                            { year: "Now", title: "Global Standards", desc: "ISO 19650 certified and internationally recognised by power and infrastructure giants." },
                        ].map((item, i) => (
                            <FadeIn key={i} delay={i * 0.1}>
                                <div className={`p-8 rounded-2xl border h-full ${i === 1 ? "bg-primary text-white border-primary" : "bg-white border-slate-100 shadow-sm"}`}>
                                    <p className={`font-display font-bold text-5xl mb-4 ${i === 1 ? "text-accent" : "text-primary"}`} style={{ letterSpacing: "-0.05em" }}>
                                        {item.year}
                                    </p>
                                    <h3 className={`font-display font-bold text-xl mb-3 ${i === 1 ? "text-white" : "text-primary"}`} style={{ letterSpacing: "-0.02em" }}>
                                        {item.title}
                                    </h3>
                                    <p className={`font-body text-sm leading-relaxed ${i === 1 ? "text-white/65" : "text-slate-500"}`}>
                                        {item.desc}
                                    </p>
                                </div>
                            </FadeIn>
            ...
            <section className="bg-white py-28 text-center">
                <FadeIn className="container mx-auto px-6 max-w-3xl">
                    ...
                    <Link
                        href="/contact"
                        className="group inline-flex items-center gap-3 bg-accent hover:bg-primary text-white font-body font-semibold px-10 py-4 rounded-full transition-all shadow-xl shadow-accent/30 text-base"
                    >
